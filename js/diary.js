...
export function initDiaryPage() {
  ...
  // Multi-tier fallback content catalog resolver unblocking legacy tracking logs
  async function resolveAnimeDetailsPayload(animeId) {
    const numericId = parseInt(animeId);
    
    // Tier 1: Process native AniList GraphQL dataset queries
    if (numericId > 100000) {
      try {
        const queryStr = `
          query ($id: Int) {
        ...
        const res = await fetch("https://graphql.anilist.co", {
          ...
          body: JSON.stringify({ query: queryStr, variables: { id: numericId } })
        });
        if (res.ok) {
          const json = await res.json();
          const media = json.data?.Media;
          
          let statusStr = media?.status || "Unknown";
          if (statusStr === "FINISHED") statusStr = "Finished Airing";
          if (statusStr === "RELEASING") statusStr = "Currently Airing";
          if (statusStr === "NOT_YET_RELEASED") statusStr = "Upcoming";
        ...
        console.warn("AniList core log miss, looking up historical metadata references...", err);
      }
    }

    // Tier 2: Process fallback requests to catch 404/legacy Jikan identifier codes safely
    try {
      ...
      if (res.ok) {
        ...
        return {
          ...
          imgUrl: json.data?.images?.jpg?.large_image_url || "",
          status: json.data?.status || "Status"
        };
      }
    } catch (err) {
      console.error("All media catalog indexes exhausted for key reference: ", animeId, err);
    }

    // Return elegant fallback mock data instead of crashing layout on deleted items (404)
    return { title: `Archived Title (${animeId})`, imgUrl: "", status: "Completed Log" };
  }

  // CRITICAL SECURITY RULE FIX: Nest the entire querying routine safely inside the Auth tracking callback wrapper
  onAuthStateChanged(auth, async (user) => {
    ...
    try {
      ...
      // RUN METRIC DATA CALCULATIONS FOR ANALYTICS CARD BOARD
      ...
      if (analyticsBox) {
        analyticsBox.innerHTML = `
          <div class="analytics-dashboard-card" style="grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); width: 100%;">
      ...
      for (const entry of data) {
        try {
          const a = await resolveAnimeDetailsPayload(entry.animeId);
          const div = document.createElement("div");
          div.className = "anime-card glass reveal visible";
          ...
          div.innerHTML = `
            ...
            ${entry.note ? `<p class="diary-note" style="margin: 6px 14px 0; font-size:13px; opacity:0.85; text-align:left;">${entry.note}</p>` : ""}
          `;
...
function openCustomDeleteModal(entry, cardEl) {
  ...
  card.innerHTML = `
    ...
    <p style="font-size: 13px; color: var(--text-muted); margin-bottom: 20px; line-height: 1.6; text-align:left;">
